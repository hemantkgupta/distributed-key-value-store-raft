...
The architectural recommendation in the 2026 deep-research report (`raw/articles/distributed-key-value-store-deep-research-report.md` in the wiki repo) is **range-partitioned, single-leader-per-partition Multi-Raft**. This repo builds exactly that.
...
|---|---|---|
| `kv-common` | Rewritten | Value types, NodeId, MutationRecord, HLC |
...
| `kv-storage-toy-lsm` | **Forked verbatim** | Educational toy LSM |
| `kv-raft-core` | New (Phase 1) | Raft consensus primitive: log, leader election, AppendEntries |
| `kv-raft-kv` | New (Phase 2) | KV state machine running on top of `kv-raft-core` |
| `kv-partitioning-range` | New (Phase 3) | Range descriptors, range scans, split-point selection |
| `kv-multi-raft` | New (Phase 3) | Multi-Raft cluster glue + range-to-group mapping + split coordination |
| `kv-leader-lease` | New (Phase 4) | Time-based leader leases with bounded clock skew |
| `kv-mvcc` | New (Phase 4) | MVCC versioned storage layout + snapshot read |
| `kv-membership` | New (Phase 5) | Joint consensus per Raft group + cluster-level SWIM/gossip |
| `kv-node` | New | Node runtime + HTTP transport (Multi-Raft-shaped) |
| `kv-client` | New (Phase 2+) | Leader-aware client with retry-on-not-leader, hedged reads |
...
Java 17 (`jenv local 17` if jenv is in use). System Gradle ≥ 8.0. No gradle wrapper checked in (matching the AP companion's convention).
